...
var (
	...
	// these goweb.Controller's methods shouldn't reflect to AutoRouter
...
func (p *ControllerRegister) Add(pattern string, c ControllerInterface, mappingMethods ...string) {
	...
	route.routerType = routerTypegoweb
...
// example goweb.AddAuto(&MainContorlller{}),
...
// example goweb.AddAutoPrefix("/admin",&MainContorlller{}),
// MainController has method List and Page.
...
func (p *ControllerRegister) geturl(t *Tree, url, controllName, methodName string, params map[string]string, httpMethod string) (bool, string) {
	...
	for _, l := range t.leaves {
		if c, ok := l.runObject.(*controllerInfo); ok {
			if c.routerType == routerTypegoweb &&
