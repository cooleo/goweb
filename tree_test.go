...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "cooleo")
		ctx := context.NewContext()
		obj := tr.Match(r.requesturl, ctx)
		if obj == nil || obj.(string) != "cooleo" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "cooleo")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "cooleo")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
	ctx := context.NewContext()
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "cooleo")
	tr.AddRouter("/shop/:sd/account", "cooleo")
	t3 := NewTree()
