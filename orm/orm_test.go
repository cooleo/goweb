...
func TestCRUD(t *testing.T) {
	...
	err = dORM.Read(u)
	throwFailNow(t, err)
	throwFail(t, AssertIs(u.UserName, "cooleo"))
	throwFail(t, AssertIs(u.Profile.ID, profile.ID))

	u = &User{UserName: "cooleo", Password: "pass"}
...
func TestCustomField(t *testing.T) {
	...
	throwFailNow(t, AssertIs(user.Extra.Name, "goweb"))
...
func TestOrderBy(t *testing.T) {
	...
	num, err = qs.OrderBy("-profile__age").Filter("user_name", "cooleo").Count()
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
}
...
func TestAll(t *testing.T) {
	...
	throwFail(t, AssertIs(users[1].UserName, "cooleo"))
	...
	throwFailNow(t, AssertIs(users2[1].UserName, "cooleo"))
...
func TestValuesFlat(t *testing.T) {
	...
	if num == 3 {
		...
		throwFail(t, AssertIs(list[1], "cooleo"))
...
func TestRelatedSel(t *testing.T) {
	...
	throwFailNow(t, AssertIs(posts[1].User.UserName, "cooleo"))
	throwFailNow(t, AssertIs(posts[2].User.UserName, "cooleo"))
...
func TestReverseQuery(t *testing.T) {
	...
	profile = Profile{}
	err = dORM.QueryTable("user_profile").Filter("User__UserName", "cooleo").One(&profile)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(profile.Age, 30))
	...
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "cooleo"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "cooleo").Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "cooleo"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "cooleo").RelatedSel().Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "cooleo"))
	...
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "cooleo").All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	tags = []*Tag{}
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "cooleo").RelatedSel().All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	throwFailNow(t, AssertIs(tags[0].BestPost.User.UserName, "cooleo"))
}
...
func TestLoadRelated(t *testing.T) {
	...
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(len(user.Posts), 2))
	throwFailNow(t, AssertIs(user.Posts[0].User.UserName, "cooleo"))
	...
	throwFailNow(t, AssertIs(profile.User.UserName, "cooleo"))
	...
	throwFailNow(t, AssertIs(post.User.UserName, "cooleo"))
	...
	throwFailNow(t, AssertIs(post.Tags[0].BestPost.User.UserName, "cooleo"))
...
func TestQueryRows(t *testing.T) {
	...
	throwFailNow(t, AssertIs(usernames[1], "cooleo"))
