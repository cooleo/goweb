...
func init() {
	...
	BConfig = &Config{
		...
		ServerName:          "gowebServer:" + VERSION,
		RecoverPanic:        true,
		...
		WebConfig: WebConfig{
			AutoRender:             true,
			EnableDocs:             false,
			FlashName:              "goweb_FLASH",
			FlashSeparator:         "gowebFLASH",
	...
	if !utils.FileExists(appConfigPath) {
		AppConfig = &gowebAppConfig{innerConfig: config.NewFakeConfig()}
		return
	}
...
func parseConfig(appConfigPath string) (err error) {
	...
	if envRunMode := os.Getenv("goweb_RUNMODE"); envRunMode != "" {
...
type gowebAppConfig struct {
	innerConfig config.Configer
}

func newAppConfig(appConfigProvider, appConfigPath string) (*gowebAppConfig, error) {
	ac, err := config.NewConfig(appConfigProvider, appConfigPath)
	if err != nil {
		return nil, err
	}
	return &gowebAppConfig{ac}, nil
}

func (b *gowebAppConfig) Set(key, val string) error {
...
func (b *gowebAppConfig) String(key string) string {
...
func (b *gowebAppConfig) Strings(key string) []string {
...
func (b *gowebAppConfig) Int(key string) (int, error) {
...
func (b *gowebAppConfig) Int64(key string) (int64, error) {
...
func (b *gowebAppConfig) Bool(key string) (bool, error) {
...
func (b *gowebAppConfig) Float(key string) (float64, error) {
...
func (b *gowebAppConfig) DefaultString(key string, defaultVal string) string {
	if v := b.String(key); v != "" {
		return v
	}
	return defaultVal
...
func (b *gowebAppConfig) DefaultStrings(key string, defaultVal []string) []string {
...
func (b *gowebAppConfig) DefaultInt(key string, defaultVal int) int {
...
func (b *gowebAppConfig) DefaultInt64(key string, defaultVal int64) int64 {
...
func (b *gowebAppConfig) DefaultBool(key string, defaultVal bool) bool {
...
func (b *gowebAppConfig) DefaultFloat(key string, defaultVal float64) float64 {
...
func (b *gowebAppConfig) DIY(key string) (interface{}, error) {
	return b.innerConfig.DIY(key)
}

func (b *gowebAppConfig) GetSection(section string) (map[string]string, error) {
	return b.innerConfig.GetSection(section)
}

func (b *gowebAppConfig) SaveConfigFile(filename string) error {
