...
func Upgrade(lasttime int64) error {
	...
	for _, v := range sm {
		if v.created > lasttime {
			goweb.Info("start upgrade", v.name)
			v.m.Reset()
			v.m.Up()
			err := v.m.Exec(v.name, "up")
			if err != nil {
				goweb.Error("execute error:", err)
				time.Sleep(2 * time.Second)
				return err
			}
			goweb.Info("end upgrade:", v.name)
			i++
		}
	}
	goweb.Info("total success upgrade:", i, " migration")
	time.Sleep(2 * time.Second)
	return nil
}
...
func Rollback(name string) error {
	if v, ok := migrationMap[name]; ok {
		...
		goweb.Info("end rollback")
		time.Sleep(2 * time.Second)
		return nil
	}
	goweb.Error("not exist the migrationMap name:" + name)
	time.Sleep(2 * time.Second)
...
func Reset() error {
	...
	for j := len(sm) - 1; j >= 0; j-- {
		...
		if isRollBack(v.name) {
			goweb.Info("skip the", v.name)
			time.Sleep(1 * time.Second)
			continue
		}
		goweb.Info("start reset:", v.name)
		v.m.Reset()
		v.m.Down()
		...
		i++
		goweb.Info("end reset:", v.name)
	}
	goweb.Info("total success reset:", i, " migration")
	time.Sleep(2 * time.Second)
	return nil
}
...
func isRollBack(name string) bool {
	...
	if err != nil {
		goweb.Info("get name has error", err)
		return false
	}
	if num <= 0 {
		return false
