...
// Handler goweb filter handler for serve captcha image
func (c *Captcha) Handler(ctx *context.Context) {
	...
	if len(ctx.Input.Query("reload")) > 0 {
		...
		if err := c.store.Put(key, chars, c.Expiration); err != nil {
			...
			goweb.Error("Reload Create Captcha Error:", err)
			return
		}
	} else {
	...
	if _, err := img.WriteTo(ctx.ResponseWriter); err != nil {
		goweb.Error("Write Captcha Image Error:", err)
	}
}
...
func (c *Captcha) CreateCaptchaHTML() template.HTML {
	...
	if err != nil {
		goweb.Error("Create Captcha Error:", err)
		return ""
	}
...
func NewWithFilter(urlPrefix string, store cache.Cache) *Captcha {
	...
	goweb.InsertFilter(cpt.URLPrefix+"*", goweb.BeforeRouter, cpt.Handler)

	// add to template func map
	goweb.AddFuncMap("create_captcha", cpt.CreateCaptchaHTML)
